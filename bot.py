...
async def pdisk_up(link):
    if ('pdisk' in link or 'kuklink' in link or 'kofilink' in link or 'cofilink' in link or 'bit' in link or link in 'vdshort' or link in 'vidrivers'or link in 'pdiskshortener'):
    ...
    res = requests.get(
        'http://linkapi.net/open/create_item?link_type=link&content_src=' + link + '&source=2000&cover_url='+THUMB_URL+'&api_key=' + PDISK_API_KEY + '&dir_id=0&title=' + title_pdisk + '&description=Join Our ' + 'CHANNEL' + 'For More Movies , series ,etc....')
    data = res.json()
    data = dict(data)
    print(data)
